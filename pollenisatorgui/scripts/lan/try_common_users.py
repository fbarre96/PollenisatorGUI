...
def main(apiclient, **kwargs):
    ...
    if users.strip() != "":
        with open(file_name, "w") as f:
            f.write(users+"\n")
        exec += 1
        utils.executeInExternalTerm(f"'pollex cme smb {dc} -u {file_name} -p {file_name} -d {domain} --no-bruteforce --continue-on-success'", default_target=kwargs.get("default_target", None))
