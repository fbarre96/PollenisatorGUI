...
def main(apiclient, appli, **kwargs):
    ...
    if kwargs.get("target_type").lower() == "share":
        share_info =  apiclient.find("ActiveDirectory", {"type":"share", "_id":ObjectId(kwargs.get("target_iid", ""))}, False)
    ...
    for file in share_info.get("files" ,[]):
        if file["users"] and file["flagged"]:
            ...
            user_info =  apiclient.find("ActiveDirectory", {"type":"user","username":file["users"][0][1], "domain":file["users"][0][0]}, False)
