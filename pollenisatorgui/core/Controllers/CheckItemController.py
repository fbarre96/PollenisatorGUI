...
class CheckItemController(ControllerElement):
    ...
    def doUpdate(self, values):
        ...
        """
        # Get form values
        self.model.title = values.get(
             "Title", self.model.title)
        self.model.description = values.get(
            "Description", self.model.description)
        self.model.check_type = values.get(
             "Check type", self.model.check_type)
    ...
    def doInsert(self, values):
        ...
        for command in commands_raw:
            if len(command) == 3:
                commands.append(str(command[2]))
        self.model.initialize(title, pentest_types=pentest_types, description=description, category=category, check_type=check_type, step=step, parent=parent, commands=commands, script="", defects=None, infos=None)
        # Insert in database
